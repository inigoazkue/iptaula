...
`GET /api/resolve/{ip}` hace una resolución DNS inversa (`socket.gethostbyaddr`, con `socket.setdefaulttimeout(5)` para no bloquear indefinidamente) desde el propio contenedor. Comparte estética con el ping (mismas clases CSS `ping-green`/`ping-red`/`pinging`) pero es binario: encontrado o no, sin tramos intermedios.
